...
version: 1.0.0
description: |
  Use when the user wants strategic feedback on a plan — whether the problem is
  worth solving, if scope is right, or if the approach is fundamentally wrong.
  Triggers on: 'review my plan', 'is this worth building', 'strategic review',
  'founder review', 'ceo review'.
...
22. **Failure as ambition signal** — If the plan has zero risk of failure, it has zero ambition. But distinguish between types of risk: **invisible failures** (silent data corruption, undetected edge cases, missing error handling) must be caught — these are landmines. **Visible risks** (ambitious features that might not work, bold design choices, new approaches) should be celebrated — these are bets the team makes with open eyes. A great plan takes visible risks while eliminating invisible failures. 80% of work doesn't succeed — that's the process working correctly. The willingness to kill your own work is a discipline, not a defeat.
