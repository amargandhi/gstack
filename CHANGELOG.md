...
## **Canonical engineering literature, stitched into the skill layer. Plus GPT-5.5 on day one.**

This fork lands on top of upstream v1.26.3.0 with seven branches of work grounding Claude's skill prose in named software-engineering canon, plus a GPT-5.5 model overlay released the day OpenAI shipped it. The core engine is untouched — template pipeline, browser, hooks, telemetry, gbrain all work exactly as upstream. What changed is vocabulary: Claude now names the specific smell (shallow module, pass-through method, mixed-axis commit, Feature Envy, Integration Point) rather than saying "this feels off," and /challenge + /glossary add two new analytical skills that pure-analyze without touching code.

### What shipped

Seven canonical-literature branches, each independently squash-mergeable:

1. **Canonical vocabulary** (Parnas 1972, Meyer 1988, Beck 2002 + 2024, Ousterhout 2018) — new `docs/DESIGN_TESTS.md` reference doc plus vocabulary injected into `/review` and `/plan-eng-review`. Names the specific smell instead of gesturing.
2. **Fowler's 24 code smells** — `scripts/code-smells.json` + `{{CODE_SMELLS_CHECKLIST}}` resolver wired into `/review` Step 4.5 and `/plan-eng-review`. Feature Envy, Data Clumps, Shotgun Surgery — canonical names so reviewers and authors share vocabulary.
3. **Nygard stability patterns** (Release It! 2nd ed.) — new `/cso --stability` mode audits 12 anti-patterns (Integration Points, Chain Reactions, Cascading Failures, Blocked Threads, Dogpile, etc.) and 12 counter-patterns (Timeouts, Circuit Breaker, Bulkheads). Carved out of `/cso`'s DoS exclusion.
4. **Git-guards hook** — `bin/gstack-install-git-guards` installs a PreToolUse hook blocking 7 destructive git ops on protected branches (force-push, reset --hard, clean -f, branch -D, checkout/restore ., rebase -i, --no-verify). Feature-branch force-push allowed. Opt-in.
5. **`/investigate --file-issue`** (Feathers 2004 — characterization tests first) — diagnose root cause, STOP, file GitHub issue with diagnosis + a characterization-test-first-ordered TDD plan. Refuses to file if hypothesis is speculative.
6. **`/glossary`** (Evans 2003 Ch. 2 + Ch. 14) — new skill. Builds an ubiquitous language doc with bounded contexts and Evans' 9 relationship types (Shared Kernel, Customer/Supplier, Conformist, Anticorruption Layer, Open Host Service, Published Language, Separate Ways, Partnership, Big Ball of Mud). Hard gate: docs only, no refactors.
7. **`/challenge`** (Polya 1945 — *How to Solve It*) — new skill. Stress-tests a plan across Polya's 4 stages (Understand → Devise → Carry out → Look back), 3-5 hard questions per stage with P1/P2/P3 priorities. Verdict at top: READY / OPEN QUESTIONS / CRITICAL GAPS. Pure analysis, no plan edits.

Plus a GPT-5.5 model overlay (`model-overlays/gpt-5.5.md`) documenting the April 2026 release: new reasoning_effort ladder (`low | medium | high | xhigh`), 1M API context / 400K Codex CLI, migration gotcha (Codex 0.124.0+ resets effort on model swap), honest cross-provider benchmarks (where Opus 4.7 still edges it out on SWE-bench Pro, HLE no-tools, MCP-Atlas), and gstack-specific recommendations for when to use 5.5 vs stay on 5.4 or 5.3-codex-spark. The model ID is `gpt-5.5` — not `gpt-5.5-codex` — with defensive aliasing in `scripts/models.ts` so `gpt-5.5-codex*` inputs map to `gpt-5.5`.

### Merged from upstream

Everything between v1.6.1.0 (our previous base) and v1.26.3.0 (main's current head), including: slim preamble (~25% token reduction per cold read, v1.15.0.0), real-PTY plan-mode E2E harness, Claude outside-voice skill (v1.13.0.0), gstack browser sidebar interactive Claude Code REPL with live tab awareness (v1.14.0.0), tunnel allowlist 17→26 (v1.16.0.0), `/setup-gbrain` GBrain federation surface (v1.17.0.0), browser-skills runtime (v1.20.0.0), always-prefix v<VERSION> on PR titles (v1.23.0.0), cross-platform Windows lane + Bun.which resolver (v1.24.0.0), AskUserQuestion host MCP variant resolution (v1.25.0.0), V1 transcript ingest + per-skill gbrain manifests (v1.26.0.0), `/sync-gbrain` skill + native code-surface orchestrator (v1.26.3.0), and more. See upstream CHANGELOG entries below v1.27.0.0 for details.

### For contributors

- Each of the 7 fork branches is an independent squash-mergeable commit on `feat/canonical-code-quality-vocab`. Bisect-friendly: template changes separate from regeneration, new skills separate from wiring.
- The `/challenge` and `/glossary` skills follow the Hard Gate pattern (pure analysis, no code) — a template worth reusing for future analysis skills.
- `model-overlays/gpt-5.5.md` inherits from `gpt` via `{{INHERIT:gpt}}` — the chain mechanism worked first-try for a brand-new model family.
- Three draft PRs already open against garrytan/gstack: #1230 (gpt-5.5 overlay), #1231 (/challenge), #1232 (/glossary).
