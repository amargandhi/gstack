...
2. **Fowler's 24 code smells** — `scripts/code-smells.json` + `{{CODE_SMELLS_CHECKLIST}}` resolver wired into `/review` Step 4.5. Feature Envy, Data Clumps, Shotgun Surgery — canonical names so reviewers and authors share vocabulary.
...
Everything between v1.6.1.0 (our previous base) and v1.12.2.0 (main's current head), including: `/setup-gbrain` onboarding flow, GBrain MCP integration, per-repo trust triad, sync secret scanner, prompt-injection defense stack (L1-L6), dual-listener tunnel architecture, `/pair-agent` cross-agent coordination, `/design-shotgun`, `/design-html`, `/learn` memory surface, `/plan-devex-review` + `/devex-review`, `/autoplan`, `/benchmark`, `/canary`, `/land-and-deploy`, Opus 4.7 overlay, Plan-mode review skills fix (v1.12.1.0), AskUserQuestion cadence + Pros/Cons format upgrade (v1.10.0.0), /make-pdf (v1.4.0.0), and much more. See upstream CHANGELOG entries below v1.13.0.0 for details.

### For contributors

- Each of the 7 branches is an independent squash-mergeable commit on `feat/canonical-code-quality-vocab`. Bisect-friendly: template changes separate from regeneration, new skills separate from wiring.
