...
~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"plan-eng-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'","model":"'"$_RUNTIME_MODEL"'","host":"'"$_RUNTIME_HOST"'"}' 2>/dev/null &
...
_GATE=$(~/.claude/skills/gstack/bin/gstack-model-gate "$_RUNTIME_MODEL" "plan-eng-review" 2>/dev/null || echo "OK")
if [ "${_GATE%%:*}" = "ESCALATE" ]; then
  ...
  echo ""
  echo "<system-reminder>"
  echo "MODEL_GATE: $_RUNTIME_MODEL is not suitable for /plan-eng-review."
  echo "Reason: $_SUGGEST_REASON"
  echo ""
  echo "STOP. Before continuing this skill, ask the user to either:"
  echo "  1. Re-run on a capable model: codex -m $_SUGGEST_MODEL /plan-eng-review"
  echo "  2. Or switch the model in their current harness (e.g. /model in Claude Code)"
  echo ""
  echo "Explain the trade-off briefly so they understand why. Do not proceed with /plan-eng-review on $_RUNTIME_MODEL."
  echo "</system-reminder>"
fi
...
* **Module depth** (Ousterhout, *A Philosophy of Software Design*, 2018). For each new or modified module, ask: deep (small interface, large implementation) or shallow (interface ≈ implementation)? Grep-able red flags — flag any as scope-reduction opportunities:
  - **Pass-through method** — a method that does little except call another with a similar signature (`Controller.find(id) { return service.find(id) }`). Adds interface cost without hiding anything.
  - **Pass-through variable** — a variable threaded through N layers and used only at the bottom. Every intermediate layer gets coupled to it.
  - **Thin wrappers** — reflexive Decorator chains, or a class that wraps a widely-used API by a hair. Ousterhout names Java IO streams as the canonical anti-example.
* **Design it twice** (Ousterhout, Ch. 11). For each new interface in the plan, did the author consider ≥ 2 radically different designs (minimalist / flexible / common-case / ports-and-adapters)? If the plan presents one option as inevitable, ask for alternates before accepting.
* **Conceptual integrity** (Brooks, *The Mythical Man-Month*, 1975). Does this plan add features that fight the existing design's logic, or introduce a second idiom where the codebase already has one? Flag as PLANBLOCKER — "one set of design ideas" beats more functionality.

→ Full vocabulary with source citations: `docs/DESIGN_TESTS.md`.
