...
A current fork of [garrytan/gstack](https://github.com/garrytan/gstack)
focused on three things:

1. Preserving upstream GStack's host/template architecture.
...
Current auditable surface: 49 generated skills. Verify it with:
...
open-source app. That use exposed where the fork had drifted into prose,
implicit assumptions, generated-file edits, and local convention.

This hardening pass moves the useful parts back into Garry Tan's original
architecture: thin harness, fat skills, generated docs, explicit routing, and
checks that fail when the fork's claims drift.

The goal is not to replace upstream GStack. The goal is to keep this personal
implementation useful while making its differences small, auditable, and easy
...
- Canonical code-quality vocabulary in `/review` and `/plan-eng-review`, backed by `docs/DESIGN_TESTS.md` and generated template output.
- Side-by-side host support, model overlays, thinking profiles, model gate rules, and subagent model pins.
- Deterministic model routing through `bin/gstack-model-route`, using `scripts/thinking-profiles.ts` as the source of truth.
- Structural fork checks for generated freshness, subagent policy, model-routing completeness, reachability, routing fixtures, and docs honesty under `bun run fork:doctor`.
- `routing-eval.jsonl` with no-spend structural fixtures.
- Prompt-cache anchor markers in the shared preamble plus a no-spend prompt-cost reporting harness.

This fork does not change:

- Browser daemon command semantics.
- Host config schema.
- Template generation as the source of truth for `SKILL.md`.
- Safety hook architecture.
- API-spend defaults. Benchmarks and LLM evals remain opt-in behind explicit budget approval.
...
| `/challenge` | **Plan Stress-Test** — Polya's "How to Solve It" applied to your plan. 4 stages (Understand → Devise → Carry out → Look back), 3-5 hard questions per stage with agent-recommended answers and P1/P2/P3 priorities. Verdict at top: READY / OPEN QUESTIONS / CRITICAL GAPS. Pure analysis — no plan edits, no code. Run before `/ship` when reversibility matters. |
| `/glossary` | **Ubiquitous Language** — Evans DDD applied to your codebase. Builds a domain glossary with bounded contexts and 9 relationship types (Shared Kernel, Customer/Supplier, Conformist, Anticorruption Layer, Open Host Service, Published Language, Separate Ways, Partnership, Big Ball of Mud). Hard gate: documentation only, no refactors. Surfaces "language seams" — terms meaning different things in different contexts. |
...
gstack supports one sprint or many isolated sprints at once.
...
- **[Garry Tan](https://github.com/garrytan/gstack)** - original gstack architecture and upstream project.
- **Anthropic** - Claude Code skills and agent workflow documentation.
