...
// T2: T1 + voice(full) + ask + completeness + context-recovery + claude-code-hints (claude only)
...
export function generatePreamble(ctx: TemplateContext): string {
  ...
  const sections = [
    ...
    ...(tier >= 2 ? [generateContextRecovery(ctx), generateClaudeCodeHints(ctx), generateAskUserFormat(ctx), generateCompletenessSection()] : []),
