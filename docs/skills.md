...
| [`/challenge`](#challenge) | **Adversarial Reviewer** | Polya 4-stage plan stress-test. Walks Understand → Devise → Carry out → Look back. Pure analysis, no plan edits. Verdict at top: READY / OPEN QUESTIONS / CRITICAL GAPS. |
| [`/glossary`](#glossary) | **Domain Linguist** | Evans DDD ubiquitous language doc. Bounded contexts, 9 relationship types, language seams. Docs only — no refactors. |
