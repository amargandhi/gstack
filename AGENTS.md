...
| `/challenge` | Polya 4-stage plan stress-test (Understand → Devise → Carry out → Look back). Pure analysis. |
| `/glossary` | Evans DDD: build an ubiquitous language doc with bounded contexts and 9 relationship types. Docs only. |
...
| `/operator-retro` | Reflect on how you operated Claude, Codex, and GStack: prompts, proof habits, fix loops, and missed leverage. |
| `/ai-usage-review` | Deprecated alias for `/operator-retro`; explicit invocation only. |
