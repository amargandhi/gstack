...
version: 1.0.0
description: |
  Use when the user wants code reviewed before merging — checks for safety issues,
  structural problems, and correctness. Triggers on: 'review this PR', 'code review',
  'check my diff', 'pre-landing review'.
allowed-tools:
  - Bash
  - Read
  - Edit
...
- Never rationalize away a real issue — if something looks wrong, flag it
- Don't batch all findings at the end — report critical issues immediately via Fix-First
- Verify claims in commit messages and PR descriptions against the actual code
- If diff is >500 lines, suggest splitting the PR before reviewing
- Design review is conditional on frontend files being changed — don't force it



## Adapt

- **PR is draft/WIP?** Focus on architecture and direction, not nitpicks.
- **Diff is trivial (< 20 lines)?** Skip the multi-pass review — a quick scan suffices.
- **No Greptile integration?** Skip the Greptile step entirely — it's optional.
- **Monorepo with isolated packages?** Scope the review to the affected package.
- **User asks for specific focus?** Prioritize their concern over the standard checklist.
