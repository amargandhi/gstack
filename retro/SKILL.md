...
~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"retro","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'","model":"'"$_RUNTIME_MODEL"'","host":"'"$_RUNTIME_HOST"'"}' 2>/dev/null &
...
_GATE=$(~/.claude/skills/gstack/bin/gstack-model-gate "$_RUNTIME_MODEL" "retro" 2>/dev/null || echo "OK")
if [ "${_GATE%%:*}" = "ESCALATE" ]; then
  ...
  echo ""
  echo "<system-reminder>"
  echo "MODEL_GATE: $_RUNTIME_MODEL is not suitable for /retro."
  echo "Reason: $_SUGGEST_REASON"
  echo ""
  echo "STOP. Before continuing this skill, ask the user to either:"
  echo "  1. Re-run on a capable model: codex -m $_SUGGEST_MODEL /retro"
  echo "  2. Or switch the model in their current harness (e.g. /model in Claude Code)"
  echo ""
  echo "Explain the trade-off briefly so they understand why. Do not proceed with /retro on $_RUNTIME_MODEL."
  echo "</system-reminder>"
fi
