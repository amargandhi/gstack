...
version: 2.0.0
description: |
  Use when the user wants to reflect on recent work — analyzes commit history,
  work patterns, code quality metrics with per-person breakdown. Triggers on:
  'weekly retro', 'what did we ship', 'retrospective', 'week in review',
...
- Never compare teammates negatively — focus on patterns, not people
- No generic praise ("great job team!") — be specific or say nothing
- Don't fabricate metrics if git history is unavailable — say so
- Tone: direct, data-driven, constructive. Not coddling, not harsh.



## Adapt

- **No git history available?** Skip metric collection, focus on qualitative analysis from the user.
- **Solo developer?** Skip team breakdown, focus on personal patterns and velocity.
- **Short time period?** Adjust lookback window — not everything needs a full week.
- **No previous retros?** Skip week-over-week comparison, establish baseline instead.
- **User only wants highlights?** Skip the full 14-step analysis, give a 3-bullet summary.
