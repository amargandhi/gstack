...
describe('gen-skill-docs', () => {
  ...
  test('plan-review generated preambles stay under the Option A budget', () => {
    ...
    // Ratcheted from 35000 → 45000 on this fork: preamble-bash carries
    // active-fork tracker, runtime host/model detection, model gate, and
    // dynamic overlay emission (~5K bytes of fork-specific safety/routing
    // logic). Canonical vocabulary in /plan-eng-review template adds another
    // ~1-2K. Both earn their bytes for this fork's use cases.
    for (const skill of reviewSkills) {
      const content = fs.readFileSync(skill.path, 'utf-8');
      const preamble = extractPreambleBeforeWorkflow(content, skill.markers);
      expect(Buffer.byteLength(preamble, 'utf-8')).toBeLessThan(45_000);
    }
  });
